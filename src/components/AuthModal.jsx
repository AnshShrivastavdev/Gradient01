...
export const AuthModal = ({ isOpen, onClose, initialTab = 'WORKER_LOGIN', onLoginSuccess }) => {
  const { registerWorker, loginWorker, loginAdmin } = useAuth();
  const [activeTab, setActiveTab] = useState(initialTab); // 'WORKER_LOGIN' | 'WORKER_REGISTER' | 'ADMIN_LOGIN'
  ...
  const [workerEmergencyContact, setWorkerEmergencyContact] = useState('');
  ...
  const handleWorkerLogin = async (e) => {
    ...
    if (!workerMobile || !workerEmpId) {
      setFeedback({ type: 'ERROR', message: 'PLEASE ENTER BOTH MOBILE NUMBER AND WORKER EMPLOYEE ID.' });
      return;
    }
  ...
  const handleWorkerRegister = (e) => {
    ...
    if (!workerEmpId || !workerName || !workerMobile) {
      setFeedback({ type: 'ERROR', message: 'MANDATORY: Worker ID, Name, and Mobile Number are required.' });
      return;
    }
    ...
    registerWorker({
      ...
      mobile: workerMobile,
      sector: workerSector,
      emergencyContact: workerEmergencyContact || workerMobile,
      password: workerPassword || 'worker123',
    });

    setFeedback({
      type: 'SUCCESS',
      message: `ENROLLED SUCCESSFULLY [ ${workerEmpId.toUpperCase()} ]. Worker added to live shift muster with Mobile ${workerMobile}. You can now log in.`,
    });

    // Pre-fill login inputs and switch to login tab
    setTimeout(() => {
      setActiveTab('WORKER_LOGIN');
    }, 1200);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4 font-mono select-none backdrop-blur-sm">
      <div className="bg-[#161B22] border-2 border-[#30363D] max-w-md w-full p-6 space-y-5 relative shadow-2xl">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 bg-[#30363D] text-white px-2.5 py-1 text-xs font-bold hover:bg-gray-600 transition-colors"
        >
          [ ✕ CLOSE ]
        </button>
        ...
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <span className="text-[10px] bg-[#00B4D8] text-black font-bold px-2 py-0.5 uppercase tracking-wider">
          ...
          <h3 className="text-xl font-display font-extrabold text-white uppercase tracking-tight">
            {activeTab === 'ADMIN_LOGIN' ? 'SAFETY INCHARGE ADMIN' : 'WORKER SHIFT PORTAL'}
          </h3>
        </div>

        {/* Navigation Tabs */}
        <div className="grid grid-cols-3 gap-1 bg-[#0D1117] p-1 border border-[#30363D] text-[11px] font-bold text-center">
          <button
            onClick={() => { setActiveTab('WORKER_LOGIN'); setFeedback(null); }}
          ...
          <button
            onClick={() => { setActiveTab('WORKER_REGISTER'); setFeedback(null); }}
            className={`py-1.5 border transition-colors ${
        ...
        {activeTab === 'WORKER_LOGIN' && (
          <form onSubmit={handleWorkerLogin} className="space-y-4 text-xs">
            <div className="flex justify-between items-center text-[10px] text-[#8B949E]">
              <span>AUTHENTICATE WITH REGISTERED MOBILE NUMBER</span>
              <button
                type="button"
            ...
            <div className="space-y-1">
              ...
              <div className="flex">
                <span className="bg-[#0D1117] border border-r-0 border-[#30363D] px-2.5 py-2 text-[#8B949E]">
                  +91
                </span>
                <input
                  type="tel"
                  ...
                  className="w-full bg-[#0D1117] border border-[#30363D] p-2 text-white outline-none focus:border-[#15803D]"
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <label className="text-[#8B949E] uppercase font-bold">Worker Shift PIN / Password:</label>
              <input
                type="password"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-[#15803D] text-white font-bold uppercase border border-white hover:bg-green-700 transition-colors"
            >
              {loading ? '[ VERIFYING MOBILE AUTH... ]' : '[ CHECK-IN TO UNDERGROUND SHIFT ]'}
            </button>
          </form>
        )}

        {/* 2. NEW WORKER REGISTRATION (Enrolls into digital shift muster) */}
        {activeTab === 'WORKER_REGISTER' && (
          <form onSubmit={handleWorkerRegister} className="space-y-3 text-xs">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-[#0D1117] border border-[#30363D] p-2 text-white outline-none focus:border-[#00B4D8]"
              />
            </div>
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-[#00B4D8] text-black font-bold uppercase border border-white hover:bg-cyan-400 transition-colors"
            >
        ...
        {activeTab === 'ADMIN_LOGIN' && (
          <form onSubmit={handleAdminLogin} className="space-y-4 text-xs">
            <div className="p-2.5 bg-[#2A1D0E] border border-[#B45309] text-[#F59E0B] text-[11px] space-y-1">
              <div className="font-bold uppercase">⚠️ RESTRICTED SAFETY INCHARGE ACCESS</div>
              <p className="text-[10px] text-[#8B949E]">
                Single Authorized Admin Email Policy. Only the designated SCADA Safety Officer is authorized.
              </p>
            </div>

            <div className="flex justify-between items-center text-[10px]">
              <span className="text-[#8B949E]">OFFICIAL ADMIN EMAIL:</span>
              <button
                type="button"
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-[#0D1117] border border-[#30363D] p-2 text-white outline-none focus:border-[#B45309]"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-[#B45309] text-white font-bold uppercase border border-white hover:bg-amber-700 transition-colors"
            >
              {loading ? '[ AUTHENTICATING VIA FIREBASE... ]' : '[ ACCESS SCADA COMMAND DESK ]'}
            </button>
          </form>
        )}
      </div>
